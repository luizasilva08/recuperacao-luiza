...
SELECT razao_social as fornecedor
FROM Fornecedor 
...
-- 20
SELECT F.razao_social as fornecedor, COUNT(E.id_equipamento)
FROM Equipamento as E
...
GROUP BY D.nome_departamento, C.nome_colaborador;

-- 22
SELECT E.nome_equipamento, E.descricao_equipamento, C.nome_categoria
FROM Equipamento as E
JOIN Categoria AS C ON E.id_categoria=C.id_categoria
GROUP BY C.nome_categoria, E.descricao_equipamento, E.nome_equipamento;

-- 23
SELECT E.descricao_equipamento, F.razao_social AS fornecedor, E.status_equipamento
FROM Equipamento as E
JOIN Fornecedor AS F ON E.id_fornecedor=F.id_fornecedor;
...
SELECT nome_equipamento, data_aquisicao
FROM Equipamento
WHERE data_aquisicao IN (SELECT MAX(data_aquisicao) FROM Equipamento);
